...
@Component({
    selector: 'meter-compares-graph',
    templateUrl: 'meter-compares-graph.html',
})
export class MeterComparesGraphComponent {
 
   @Input('invest-return') invest_return: number
   @Input('t-invest-return') tinvest_return: number
    ...
    draw() {
      ...
      const tinvest_return_val = this.tinvest_return / 10 ;

       d3.select('.chart-gauge-compares').selectAll('svg').remove();       
...
function renderMeterChart(invest_return,tinvest_return) {
  ...
  var el = d3.select('.chart-gauge-compares');
  ...
  */
  ...
  const linearGradient = defs.append('linearGradient').attr('id','blueGradient1')
  ...
  const linearGradientTwo = defs.append('linearGradient').attr('id','redGradient1')
