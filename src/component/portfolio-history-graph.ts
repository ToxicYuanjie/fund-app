...
export class PortfolioHisotryGraphComponent {
    ...
    benchmark_name: any;
    legend: Array<any>;
    periodReturnValue : any;
    period_list = [
        { value: d3.timeMonth.offset(time_now, -1), label: '月', index: 1 },
        { value: d3.timeMonth.offset(time_now, -3), label: '季' , index: 3 },
        { value: d3.timeMonth.offset(time_now, -6), label: '半年', index: 6 },
        { value: d3.timeMonth.offset(time_now, -12), label: '年', index: 12 },
    ]
    current_period: any
    ...
    ngOnInit() {
        ...
        if(this.ready){
            const daily_return = Object.assign({},this.daily_return)
    ...
    //时间
    timeTabPeriod(item,data){
        // let
        let index = 0
        let returnValue = Object.assign({},data);
        let itemValue = Object.assign(item.value);
        // 清除净值数据
    ...
    //根据tab周期查找
    searchPeriodData(item){
        let dateFormat = d3.timeFormat('%Y-%m-%d');
        let curPeriodDate = dateFormat(item.value)
        console.log(item.value)
        console.log(curPeriodDate)
        //Format
        if(dateFormat){
            const x_index = this.daily_return.days.findIndex(p => curPeriodDate == p)
            //是否找到对应下标
            if(x_index > 0){
                return x_index;
            }else{
                //迭代向内查找
                for(let i=0; i < 3 ; i++){
                    let thisValue = new Date(item.value.getTime() + (24 * 60 * 60 * 1000) * i+1 ) ;
                    let unPeriodDate = dateFormat(thisValue)
                    let un_index = this.daily_return.days.findIndex(p => unPeriodDate == p)
                    //是否找到
                    if(un_index > 0){
                        return un_index;
                    }
